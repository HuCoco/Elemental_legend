...
bool Fire_4::Cast(cocos2d::Layer *_tag, const cocos2d::Vec2 _pos, int& _num)
{
    ...
    int damage = BattleSystem::getInstance()->getPlayer()->getState()->GetAttack()*_addBuff;
    _addBuff += 0.05;
    if(_addBuff > 2.0f)
    {
        _addBuff = 2.0f;
