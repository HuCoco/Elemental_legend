...
bool FireConfig::init()
{
    ...
    _touchListener->onTouchBegan = CC_CALLBACK_2(FireConfig::onTouchBegan,this);
    _touchListener->onTouchMoved = CC_CALLBACK_2(FireConfig::onTouchMoved,this);
    _touchListener->onTouchEnded = CC_CALLBACK_2(FireConfig::onTouchEnded,this);
    ...
    _okbtn = cocos2d::MenuItemImage::create("UI/ok_btn.png", "UI/ok_btn.png", std::bind(&FireConfig::close,this));
    ...
    int TelentData = GetTalentDataByUsedData(Parser::XML::getInstance()->GetData(Parser::USER_DATA_TYPE::COST_FIRE));
    
    for(int i = 0 ; i < TelentData ; i++)
    {
        std::string path = cocos2d::StringUtils::format("skillicon/fireskillicon_%d.png",i+1);
        TALENT_INFO pRet;
        pRet.Num = i+1;
    ...
    SkillVector = JsonParser::getInstance()->GetPlayerSkillData(SELECT_FIRE);
    
    int fire_no = Parser::XML::getInstance()->GetData(Parser::USER_DATA_TYPE::TALENT_FIRE);
    if(fire_no == 1)
    {
        fire_no = 2;
    }
    _name = cocos2d::Label::createWithTTF(SkillVector->at(fire_no-2).Name, "fonts/black.ttf", 60);
    _name->setPosition(_floor->getContentSize().width/2,_floor->getContentSize().height/20*17);
    
    _floor->addChild(_name);
    
    _cost = cocos2d::Label::createWithTTF(SkillVector->at(fire_no-2).Cost, "fonts/black.ttf", 40);
    ...
    _introduction = cocos2d::Label::createWithTTF(SkillVector->at(fire_no-2).Info, "fonts/black.ttf", 32);
    ...
    if(Parser::XML::getInstance()->GetData(Parser::USER_DATA_TYPE::TALENT_FIRE) == 1)
    {
        _name->setString("未选择天赋");
        _cost->setString("------");
        _introduction->setString("");
    }
    fire_no = Parser::XML::getInstance()->GetData(Parser::USER_DATA_TYPE::TALENT_FIRE);
    for(int i = 0 ; i < (int)_talentpool.size();i++)
    {
        if(fire_no-1 == _talentpool.at(i).Num)
        {
            _talentpool.at(i)._talent->setScale(1.2f);
        }
    }
    
    int Used_data = Parser::XML::getInstance()->GetData(Parser::USER_DATA_TYPE::COST_FIRE);
    std::string usedstr = cocos2d::StringUtils::format("当前已使用 %d 次火元素",Used_data);
    ...
    std::string usedtipsstr = cocos2d::StringUtils::format("解锁新天赋还需使用 %d 次火元素",NextTelentNeed);
...
bool FireConfig::onTouchBegan(cocos2d::Touch *_touch, cocos2d::Event *_event)
{
    return true;
}

void FireConfig::onTouchMoved(cocos2d::Touch *_touch, cocos2d::Event *_event)
{
    
}

void FireConfig::onTouchEnded(cocos2d::Touch *_touch, cocos2d::Event *_event)
{
    for(int i = 0 ; i < (int)_talentpool.size();i++)
    {
        if(_talentpool.at(i)._talent->getBoundingBox().containsPoint(_floor->convertToNodeSpace(_touch->getLocation())))
        {
            ...
            Parser::XML::getInstance()->UpdateData(Parser::USER_DATA_TYPE::TALENT_FIRE, _talentpool.at(i).Num+1);
        }
    }
}
