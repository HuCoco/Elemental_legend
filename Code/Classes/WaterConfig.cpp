...
bool WaterConfig::init()
{
    ...
    _touchListener->onTouchBegan = CC_CALLBACK_2(WaterConfig::onTouchBegan,this);
    _touchListener->onTouchMoved = CC_CALLBACK_2(WaterConfig::onTouchMoved,this);
    _touchListener->onTouchEnded = CC_CALLBACK_2(WaterConfig::onTouchEnded,this);
    ...
    _okbtn = cocos2d::MenuItemImage::create("UI/ok_btn.png", "UI/ok_btn.png", std::bind(&WaterConfig::close,this));
    ...
    int TelentData = GetTalentDataByUsedData(Parser::XML::getInstance()->GetData(Parser::USER_DATA_TYPE::COST_WATER));
    
    for(int i = 0 ; i < TelentData ; i++)
    {
        std::string path = cocos2d::StringUtils::format("skillicon/waterskillicon_%d.png",i+1);
        TALENT_INFO pRet;
        pRet.Num = i+1;
    ...
    SkillVector = JsonParser::getInstance()->GetPlayerSkillData(SELECT_WATER);
    
    int water_no = Parser::XML::getInstance()->GetData(Parser::USER_DATA_TYPE::TALENT_WATER);
    if(water_no == 1)
    {
        water_no = 2;
    }
    
    _name = cocos2d::Label::createWithTTF(SkillVector->at(water_no-2).Name, "fonts/black.ttf", 60);
    _name->setPosition(_floor->getContentSize().width/2,_floor->getContentSize().height/20*17);
    //_talentpool.at(water_no-1)->setScale(1.2f);
    _floor->addChild(_name);
    
    
    _cost = cocos2d::Label::createWithTTF(SkillVector->at(water_no-2).Cost, "fonts/black.ttf", 40);
    ...
    _introduction = cocos2d::Label::createWithTTF(SkillVector->at(water_no-2).Info, "fonts/black.ttf", 32);
    ...
    if(Parser::XML::getInstance()->GetData(Parser::USER_DATA_TYPE::TALENT_WATER) == 1)
    {
        _name->setString("未选择天赋");
        _cost->setString("------");
        _introduction->setString("");
    }
    water_no = Parser::XML::getInstance()->GetData(Parser::USER_DATA_TYPE::TALENT_WATER);
    for(int i = 0 ; i < (int)_talentpool.size();i++)
    {
        if(water_no-1 == _talentpool.at(i).Num)
        {
            _talentpool.at(i)._talent->setScale(1.2f);
        }
    }
    
    int Used_data = Parser::XML::getInstance()->GetData(Parser::USER_DATA_TYPE::COST_WATER);
    std::string usedstr = cocos2d::StringUtils::format("当前已使用 %d 次水元素",Used_data);
    ...
    std::string usedtipsstr = cocos2d::StringUtils::format("解锁新天赋还需使用 %d 次水元素",NextTelentNeed);
...
bool WaterConfig::onTouchBegan(cocos2d::Touch *_touch, cocos2d::Event *_event)
{
    return true;
}

void WaterConfig::onTouchMoved(cocos2d::Touch *_touch, cocos2d::Event *_event)
{
    
}

void WaterConfig::onTouchEnded(cocos2d::Touch *_touch, cocos2d::Event *_event)
{
    for(int i = 0 ; i < (int)_talentpool.size();i++)
    {
        if(_talentpool.at(i)._talent->getBoundingBox().containsPoint(_floor->convertToNodeSpace(_touch->getLocation())))
        {
            ...
            Parser::XML::getInstance()->UpdateData(Parser::USER_DATA_TYPE::TALENT_WATER, _talentpool.at(i).Num+1);
        }
    }
}
